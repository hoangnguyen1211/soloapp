...
import { COLOR_RED, COLOR_WHITE } from '../../constants/ColorConstants';
import { Icon } from 'react-native-elements';
import QuestionButton from './QuestionButton';

const { width } = Dimensions.get('window');

export default class QuestionError extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
    ...
    _onRandomQuiz = () => {
        const { randomQuiz } = this.props;
        this._hideErrorNotify();
        randomQuiz();
    }
    ...
    render() {
        let bgColor = this.state.bgColor.interpolate({
            inputRange: [0, 4, 6],
            outputRange: [COLOR_WHITE, COLOR_WHITE, COLOR_RED]
        });
        ...
        return (
            <Animated.View style={[styles.container,  { width: width, bottom: xValue }]}>
                ...
                <QuestionButton
                    ...
                    style={{ marginTop: 40 }}
                    funcHandler={this._onRandomQuiz}
                />
            </Animated.View>
        )
    }
